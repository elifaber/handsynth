...
def audio_callback(outdata, frames, time, status):
    ...
    try:
        ...
        for i in range(frames):
            ...
            delay_output[i] = (1 - wet_percent) * current_sample + wet_percent * delay_sample  # Mix dry and wet
...
def audio_thread(stop_event):
    ...
    try:
        ...
        smoothed_lowpass_cutoff = lowpass_cutoff  # Initialize cutoff
        prev_filtered_sample = 0.0  # Initialize the previous filtered sample for the filter
        ...
        while not stop_event.is_set():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    # Calculate distance between thumb tip and index finger base for feedback control
                    index_base = hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_MCP]
                    feedback_distance = np.sqrt((thumb_tip.x - index_base.x) ** 2 + (thumb_tip.y - index_base.y) ** 2)

                    # Map this distance to a feedback level range
                    ...
                    # Calculate distance between fourth finger tip and its MCP
                    ...
                    fourth_distance = np.sqrt((fourth_finger_tip.x - fourth_finger_mcp.x) ** 2 + (fourth_finger_tip.y - fourth_finger_mcp.y) ** 2) 

                    # Map this distance to a cutoff frequency range (e.g., 200 Hz to 5000 Hz)
                    min_cutoff_distance = 0.0
                    max_cutoff_distance = 0.2  
